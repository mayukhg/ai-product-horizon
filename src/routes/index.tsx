...
import { useAgentTrajectory, useEvalMetrics, useModelRouting, useRemediateApprove, useTriage } from "@/hooks/useCyberRisk";
import type { AgentTrajectoryResponse, EvalMetricsResponse, TriageResponse } from "@/services/api";
...
const trajectoryIcons: Record<string, LucideIcon> = {
  plan: BrainCircuit,
  tool_call: Terminal,
  retrieval: Database,
...
function HorizonDashboard() {
  ...
  const trajectoryQuery = useAgentTrajectory();
  const routingQuery = useModelRouting();
  const evalQuery = useEvalMetrics();
  const approveMutation = useRemediateApprove();
  ...
  const backendOffline = triageQuery.isError || trajectoryQuery.isError;
  const effectiveDataState = backendOffline && dataState === "live" ? "offline" : dataState;
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="relative z-10 min-h-screen lg:pl-64">
        ...
        <div className="mx-auto max-w-[1600px] px-4 py-5 sm:px-6 lg:px-8">
          {backendOffline && effectiveDataState !== "offline" && <BackendOfflineBanner onRetry={() => { triageQuery.refetch(); trajectoryQuery.refetch(); }} />}
          {effectiveDataState === "drift" && <DriftBanner />}
          {effectiveDataState === "offline" ? (
            <OfflineState onRetry={() => { setDataState("live"); triageQuery.refetch(); trajectoryQuery.refetch(); }} cachedAssets={triageQuery.data?.summary.asset_count} />
          ) : effectiveDataState === "loading" ? (
            <LoadingState />
          ) : view === "eval" ? (
            <EvalStudio drift={effectiveDataState === "drift"} evalData={evalQuery.data} isLoading={evalQuery.isLoading} />
          ) : view === "roadmap" ? (
            <Roadmap />
          ) : (
            <ResidentView
              onReview={() => setApprovalOpen(true)}
              approved={approved || approveMutation.isSuccess}
              triage={triageQuery.data}
              trajectory={trajectoryQuery.data}
              routes={routingQuery.data?.routes}
              isLoading={triageQuery.isLoading || trajectoryQuery.isLoading}
            />
          )}
        </div>
      </main>
...
function ResidentView({
  ...
  trajectory?: AgentTrajectoryResponse;
  routes?: Array<{ label: string; model: string; detail: string; active: boolean }>;
  isLoading?: boolean;
}) {
  const summary = triage?.summary;
  const vulnerabilities = triage?.vulnerabilities ?? fallbackVulnerabilities;
  const trajectorySteps = trajectory?.steps
    ? trajectory.steps.map((step) => ({
        ...
        detail: step.detail,
        time: step.time,
        icon: trajectoryIcons[step.step_type] ?? CircleDot,
      }))
    : fallbackTrajectory;
  const activeRoutes = routes ?? [
    { label: "CVE lookup", model: "Smart Intern", detail: "On-prem · 22× cost saving", active: true },
    { label: "Attack-chain synthesis", model: "PhD Reasoner", detail: "Escalated · high complexity", active: false },
  ];
  ...
  return (
    <div className="animate-fade-up space-y-5">
      <section className="flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
        <div>
          <div className="mb-2 flex items-center gap-2"><span className="status-pill border-primary/30 bg-primary/10 text-primary"><Sparkles className="size-3" /> Active pilot</span><span className="font-mono text-[10px] text-muted-foreground">SCAN / {summary?.scan_id ?? "CR-0922-0048"}</span></div>
        ...
        <div className="flex flex-wrap gap-2">
          <MetricBadge label="Groundedness" value={`${summary?.kpis.groundedness_pct ?? 97.2}%`} tone="success" />
          <MetricBadge label="Latency P95" value={`${summary?.kpis.latency_p95_ms ?? 684} ms`} tone="cyan" />
          <MetricBadge label="Model cost / 1k" value={`$${(summary?.kpis.model_cost_per_1k ?? 0.84).toFixed(2)}`} tone="purple" />
        </div>
      </section>
        ...
        <div className="panel overflow-hidden">
          ...
          <div className="p-4 sm:p-5">
            <div className="mb-5 flex items-start gap-3 border-b border-border pb-4">
              ...
              <div><p className="text-sm font-medium">{trajectory?.title ?? "Investigate critical scan delta"}</p><p className="mt-1 font-mono text-[10px] text-muted-foreground">RUN-ID {trajectory?.run_id ?? "7f4c-92a1"} · {trajectory?.region ?? "prod-us-east"} · autonomous level {trajectory?.autonomy_level ?? 2}</p></div>
            </div>
            <div className="relative space-y-0">
              <div className="absolute bottom-7 left-[17px] top-7 w-px bg-border" />
              {trajectorySteps.map((step, index) => {
                const Icon = step.icon;
                return <div key={step.id} className="group relative flex gap-3 py-2.5"><div className={cn("relative z-10 grid size-9 shrink-0 place-items-center rounded-md border bg-panel transition-colors", index === trajectorySteps.length - 1 ? "border-primary text-primary shadow-status-cyan" : "border-success/30 text-success")}><Icon className="size-4" /></div><div className="min-w-0 flex-1 rounded-md border border-transparent px-2 py-0.5 transition-colors group-hover:border-border group-hover:bg-muted/30"><div className="flex items-center justify-between gap-2"><p className="text-xs font-semibold"><span className="mr-2 font-mono text-[9px] text-muted-foreground">{step.id}</span>{step.title}</p><span className="font-mono text-[9px] text-muted-foreground">{step.time}</span></div><p className="mt-1 truncate font-mono text-[10px] text-muted-foreground">{step.detail}</p>{index === 2 && <WorkerAgents workers={trajectory?.workers} />}</div></div>;
              })}
            </div>
          </div>
        </div>
          ...
          <div className="panel overflow-hidden">
            ...
            <div className="space-y-3 p-4">
              {activeRoutes.map((route) => (
                <RoutingRow key={route.label} label={route.label} model={route.model} detail={route.detail} active={route.active} />
              ))}
            </div>
          </div>
          <div className="panel overflow-hidden">
            ...
            <div className="grid grid-cols-2 divide-x divide-y divide-border">
              {[['Scope', summary?.scope ?? 'AWS prod'],['Scanner', summary?.scanner ?? 'QScanner-07'],['Policies', summary?.policies ?? 'CIS + PCI'],['Last delta', `${summary?.last_delta_sec ?? 42} sec`]].map(([label,value]) => <div key={label} className="p-3"><p className="text-[9px] uppercase text-muted-foreground">{label}</p><p className="mt-1 text-xs font-medium">{value}</p></div>)}
            </div>
          </div>
        </div>
      </section>
...
function ScoreCard({ score = 742, delta = 28 }: { score?: number; delta?: number }) {
  return <div className="panel relative overflow-hidden p-4"><div className="absolute inset-y-0 right-0 w-1 bg-danger" /><div className="flex items-start justify-between"><div><p className="text-[10px] uppercase text-muted-foreground">Enterprise TruRisk</p><div className="mt-1 flex items-end gap-2"><span className="font-mono text-3xl font-semibold">{score}</span><span className="mb-1 text-[10px] text-muted-foreground">/ 1000</span></div></div><div className="grid size-10 place-items-center rounded-full border-2 border-danger/50 text-danger"><TrendingDown className="size-4" /></div></div><div className="mt-3 flex items-center gap-2 text-[10px]"><span className="text-danger">↑ {delta} risk</span><span className="text-muted-foreground">in 24 hours</span></div></div>;
}
...
function WorkerAgents({ workers }: { workers?: Array<{ name: string; detail: string; tone: "success" | "primary" | "agent" }> }) {
  const items = workers ?? [
    { name: "Asset mapper", detail: "12 nodes", tone: "success" as const },
    { name: "Threat intel", detail: "3 feeds", tone: "primary" as const },
    { name: "Policy verifier", detail: "CIS 2.0", tone: "agent" as const },
  ];
  return <div className="mt-3 grid gap-2 sm:grid-cols-3">{items.map((item) => <div key={item.name} className="rounded-md border border-border bg-background px-2.5 py-2"><div className="flex items-center gap-1.5"><span className={cn("size-1.5 rounded-full", item.tone === "success" ? "bg-success" : item.tone === "agent" ? "bg-agent" : "bg-primary")} /><p className="text-[9px] font-semibold">{item.name}</p></div><p className="mt-1 font-mono text-[8px] text-muted-foreground">{item.detail}</p></div>)}</div>;
}
...
function VulnerabilityTable({ vulnerabilities }: { vulnerabilities: Array<{ cve: string; product: string; assets: number; cvss: string; state: string; severity: string }> }) {
...
function EvalStudio({ drift, evalData, isLoading }: { drift: boolean; evalData?: EvalMetricsResponse; isLoading?: boolean }) {
  ...
  return <div className="animate-fade-up space-y-5"><section className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><span className="status-pill border-agent/30 bg-agent/10 text-agent"><BrainCircuit className="size-3" /> Production intelligence</span><h1 className="mt-3 text-2xl font-semibold sm:text-3xl">AI PRD & Eval Studio</h1><p className="mt-1 text-sm text-muted-foreground">Continuous evaluation for CyberRisk Resident model behavior.</p></div><div className="flex gap-2"><MetricBadge label="Eval runs" value={(evalData?.total_runs ?? 18420).toLocaleString()} tone="cyan" /><MetricBadge label="Pass rate" value={passRate} tone={drift ? "purple" : "success"} /></div></section><section className="grid gap-5 xl:grid-cols-3"><div className="panel xl:col-span-2"><PanelHeader icon={Activity} title="RAG Triad · 7 hour window" aside={<span className="font-mono text-[9px] text-muted-foreground">Threshold 90%</span>} /><div className="h-[320px] p-4"><ResponsiveContainer width="100%" height="100%"><LineChart data={chartData}><CartesianGrid stroke="var(--chart-grid)" vertical={false} /><XAxis dataKey="time" stroke="var(--muted-foreground)" fontSize={9} tickLine={false} axisLine={false} /><YAxis domain={[75,100]} stroke="var(--muted-foreground)" fontSize={9} tickLine={false} axisLine={false} /><Tooltip contentStyle={{ background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 6, fontSize: 11 }} /><Line type="monotone" dataKey="retrieval" stroke="var(--chart-cyan)" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="groundedness" stroke="var(--chart-purple)" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="relevance" stroke="var(--chart-amber)" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer></div><div className="flex flex-wrap gap-4 border-t border-border px-4 py-3">{[["Retrieval Quality","bg-primary"],["Groundedness","bg-agent"],["Answer Relevance","bg-warning"]].map(([label,color]) => <div key={label} className="flex items-center gap-2 text-[10px] text-muted-foreground"><span className={cn("size-2 rounded-full", color)} />{label}</div>)}</div></div><div className="panel"><PanelHeader icon={AlertTriangle} title="Quality drift telemetry" /><div className="p-4"><div className="h-40"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData}><defs><linearGradient id="latency" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="var(--chart-red)" stopOpacity={0.3}/><stop offset="95%" stopColor="var(--chart-red)" stopOpacity={0}/></linearGradient></defs><Area type="monotone" dataKey="latency" stroke="var(--chart-red)" fill="url(#latency)" /><XAxis dataKey="time" hide /><Tooltip contentStyle={{ background: "var(--panel)", border: "1px solid var(--border)", fontSize: 11 }} /></AreaChart></ResponsiveContainer></div><div className="mt-3 space-y-3">{telemetry.map((item) => {
    const [label, value, status] = Array.isArray(item) ? item : [item.label, item.value, item.status];
    return <div key={label} className="flex items-center justify-between border-b border-border pb-3 last:border-0"><div><p className="text-[10px] text-muted-foreground">{label}</p><p className="mt-1 font-mono text-sm">{value}</p></div><span className={cn("status-pill", status === "Healthy" ? "border-success/30 bg-success/10 text-success" : status === "Alert" ? "border-danger/30 bg-danger/10 text-danger" : "border-warning/30 bg-warning/10 text-warning")}>{status}</span></div>;
  })}</div></div></div></section><EvalRuns runs={evalData?.recent_runs} /></div>;
}

function EvalRuns({ runs }: { runs?: Array<{ id: string; name: string; score: string; state: string }> }) {
  const items = runs ?? [
    { id: "CR-EVAL-441", name: "CVE response fidelity", score: "97.4%", state: "Passed" },
    { id: "CR-EVAL-440", name: "Adversarial context", score: "91.2%", state: "Passed" },
    { id: "CR-EVAL-439", name: "Citation grounding", score: "86.7%", state: "Review" },
  ];
  return <section className="panel overflow-hidden"><PanelHeader icon={CheckCircle2} title="Recent evaluation runs" aside={<Button variant="outline" size="sm" className="h-7 text-[10px]">Configure gates</Button>} /><div className="grid gap-px bg-border sm:grid-cols-3">{items.map((item) => <div key={item.id} className="bg-panel p-4"><div className="flex justify-between"><span className="font-mono text-[9px] text-muted-foreground">{item.id}</span><span className={cn("text-[9px]", item.state === "Passed" ? "text-success" : "text-warning")}>{item.state}</span></div><p className="mt-3 text-xs font-semibold">{item.name}</p><p className="mt-2 font-mono text-xl">{item.score}</p></div>)}</div></section>;
}
...
function BackendOfflineBanner({ onRetry }: { onRetry: () => void }) {
  return <div className="mb-5 flex flex-col gap-3 rounded-md border border-warning/40 bg-warning/10 p-3 sm:flex-row sm:items-center"><div className="flex items-center gap-2"><CloudOff className="size-4 text-warning" /><div><p className="text-xs font-semibold text-warning">Backend disconnected</p><p className="text-[10px] text-muted-foreground">CyberRisk API is unreachable. Cockpit layout preserved — retry to hydrate live data.</p></div></div><Button variant="outline" size="sm" className="sm:ml-auto" onClick={onRetry}><RefreshCw className="size-3.5" /> Retry API</Button></div>;
}

function OfflineState({ onRetry, cachedAssets }: { onRetry: () => void; cachedAssets?: number }) {
  return <div className="grid min-h-[70vh] place-items-center"><div className="max-w-md text-center"><div className="mx-auto grid size-14 place-items-center rounded-md border border-border bg-panel"><CloudOff className="size-6 text-muted-foreground" /></div><h1 className="mt-5 text-xl font-semibold">Live telemetry unavailable</h1><p className="mt-2 text-sm leading-6 text-muted-foreground">The latest verified snapshot remains protected. Reconnect to resume agent trajectories and scan updates.</p><div className="mt-4 rounded-md border border-border bg-panel p-3 text-left font-mono text-[10px] text-muted-foreground"><div className="flex justify-between"><span>Last trusted sync</span><span className="text-foreground">14:41:36 UTC</span></div><div className="mt-2 flex justify-between"><span>Cached assets</span><span className="text-foreground">{cachedAssets?.toLocaleString() ?? "2,847"}</span></div></div><Button onClick={onRetry} className="mt-5"><RefreshCw /> Retry connection</Button></div></div>;
}